...
- **H-Index I(274)**
	- 这题主要就是计算衡量学术影响力的一个指标**h-index**，最高效的方法就是先排序，然后依次遍历，直到遇到`i + 1 >= citations[i]`，返回`max(i, citation[i])`, 否则末尾返回`citation.size()`，由于排列的复杂度为$O(NlgN)$，后面遍历的时间复杂度为$O(n)$, 因此总的时间复杂度为$O(NlgN)$

- **H-Index II(275)**
	- 这题给定的是已经排好序的数组，因此可以用二分法进行搜索，相当于两个函数求$y= min(y_1, y_2)$，并且求$max(y)$。容易知道要么在交点出取得，要么在左端点取得，因此用二分法进行查找即可，时间复杂度为$O(lgN)$
...
- **Sort Colors(75)**
	- 这题就是把一个只包含`0`, `1`和`2`的数组按顺序整理好，类似快速排序里面的3-way paritition，把遇到的`0`往前扔，把遇到的`2`往后扔即可

- **Single Number II(137)**
	- 这题非常有意思，就是已知一个数组中除了一个元素只出现了一次外，其他元素都出现了3次，问如何在$O(N)$的时间复杂度和$O(1)$的空间复杂度内找出只出现一次的元素。
	- 最笨的方法就是对每个位置进行出现1的次数统计，最后取模3，这样拼接成的新数字就是剩下的答案
	- 第二种方法是只需要2个变量，分别表示各个位置上在遍历过程中出现1个1和出现2个1的的分布，分别为`one`和`two`，`three = two & nums[i]`，`two = (two | (one & nums[i])) & ~three`以及`one = (one | nums[i]) & ~three`
